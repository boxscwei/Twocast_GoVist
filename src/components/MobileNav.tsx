        ...
        <nav className="fixed mt-8 h-full">
          {headerNavLinks.map((link) => (
            <div key={link.title} className="px-12 py-4">
              <Link href={getLocalePath(locale, link.href)} className="text-2xl font-bold tracking-widest text-gray-900 dark:text-gray-100" onClick={onToggleNav}>
                {t(`${link.title.toLowerCase()}`)}
              </Link>
            </div>
          ))}
          <div className="py-2">Related Posts</div>
          {posts && <aside className="border-2 border-dashed border-gray-100 p-4 bg-gray-50 rounded">
            {posts.map((post) => {
              const { slug, date, title, summary, tags, language } = post;
              return (<>
                <li>
                  <Link href={getLocalePath(locale, `/blog/${slug}`)} className="text-blue-500">{title}</Link>
                </li>
              </>);
            })}
          </aside>}
