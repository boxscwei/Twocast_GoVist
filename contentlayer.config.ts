...
import { fallbackLng, secondLng } from './src/i18n/locales'
...
const computedFields: ComputedFields = {
  ...
  slug: {
    ...
    resolve: (doc) => {
      ...
      // return pathParts[pathParts.length - 1]; // 不支持多级目录，总是返回文件名（不带后缀）
      return pathParts.slice(2).join('/'); // 支持多级目录
    },
  },
  path: {
    type: 'string',
...
export const Blog = defineDocumentType(() => ({
  ...
  fields: {
    ...
    draft: { type: 'boolean' },
    trans: { type: 'boolean', default: true }, // 是否翻译
    summary: { type: 'string' },
    images: { type: 'json' },
